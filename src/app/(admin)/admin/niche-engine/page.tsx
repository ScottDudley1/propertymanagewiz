          ...
          <tbody>
            {categories.map(cat => (
              <tr key={cat.id} className="border-b border-gray-100 last:border-b-0 hover:bg-gray-50">
                <td className="px-4 py-3 font-medium text-foreground">{cat.category_name}</td>
  ...
  return (
    <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mt-2 space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="font-medium text-foreground">Score: {candidate.name}</h3>
    ...
    <tr>
      <td colSpan={7} className="px-4 py-3">
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="font-medium text-foreground">Edit: {niche.candidate?.name ?? '—'}</h3>
            <ScoreBadge score={total} />
          </div>
      ...
      {scoredNiches.length > 0 && (
        <div className="grid gap-4 md:grid-cols-3">
          <Card>
            ...
            <p className="text-3xl font-bold text-foreground">{scoredNiches.length}</p>
          </Card>
          <Card>
                ...
                {filtered.map((s, i) => (
                  editingId === s.id ? (
                    ...
                    <tr key={s.id} className="border-b border-gray-100 last:border-b-0 hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 font-medium text-foreground">{s.candidate?.name ?? '—'}</td>
...
export default function NicheEnginePage() {
  ...
  const callScoreEndpoint = useCallback(async (candidateId: number) => {
    const response = await fetch(
      `${SUPABASE_URL}/functions/v1/score-niche-candidate`,
      {
        method: 'POST',
  ...
  return (
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-2xl font-bold text-foreground">Niche Discovery Engine</h1>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Card>
            ...
            <p className="text-3xl font-bold text-foreground">{totalScored}</p>
          </Card>
        </div>
